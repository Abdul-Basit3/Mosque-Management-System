...
class LecturesManager {
    ...
    private async fetchLectures(): Promise<void> {
        try {
    ...
    private getFallbackLectures(): Lecture[] {
        return [
            {
                id: 1,
    ...
    private createLectureCard(lecture: Lecture): string {
    ...
    public playLecture(id: number): void {
    ...
    public searchLectures(query: string): void {
        const filtered = this.lectures.filter(lecture =>
...
// ===== Initialize =====
let lecturesManager: LecturesManager;

document.addEventListener('DOMContentLoaded', () => {
    lecturesManager = new LecturesManager();
    // Make it globally accessible for onclick handlers
    (window as any).lecturesManager = lecturesManager;
});
