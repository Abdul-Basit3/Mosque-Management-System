...
class EnhancedAdminDashboard {
    ...
    loadSection(section) {
        ...
        switch (section) {
            ...
            case 'all-management':
                if (pageTitle)
                    pageTitle.textContent = 'All Activities';
                this.renderAllManagement();
                break;
            case 'prayer-times':
                if (pageTitle)
                    pageTitle.textContent = 'Prayer Times Management';
                this.renderPrayerTimesManagement();
                break;
            case 'lectures':
                if (pageTitle)
                    pageTitle.textContent = 'Lectures Management';
                this.renderLecturesManagement();
                break;
            case 'courses':
                if (pageTitle)
                    pageTitle.textContent = 'Courses Management';
                this.renderCoursesManagement();
                break;
            case 'projects':
                if (pageTitle)
                    pageTitle.textContent = 'Projects Management';
                this.renderProjectsManagement();
                break;
            case 'activities':
                if (pageTitle)
                    pageTitle.textContent = 'Activities Management';
                this.renderActivitiesManagement();
                break;
            case 'executives':
                if (pageTitle)
                    pageTitle.textContent = 'Executives Management';
                this.renderExecutivesManagement();
                break;
            case 'faqs':
                if (pageTitle)
                    pageTitle.textContent = 'FAQs Management';
    ...
    showAddLectureForm() {
        ...
        form?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveLecture(new FormData(form));
        });
    }
    saveLecture(formData) {
        const lectures = JSON.parse(localStorage.getItem('lectures') || '[]');
        const newLecture = {
            id: Date.now(),
    ...
    renderGenericManagement(type, title, fields) {
        if (!this.contentElement)
            return;
    ...
    showAddForm(type, fields) {
        ...
        form?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveItem(type, new FormData(form));
        });
    }
...
//# sourceMappingURL=admin-dashboard-enhanced.js.map